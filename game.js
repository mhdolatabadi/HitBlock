...
document.addEventListener("click", () => {
  ...
  if (!isClicked) {
    setInterval(() => {
      ...
      if (
        getMeNumber(ball.style.bottom) ==
          getMeNumber(navigator.style.bottom) +
            getMeNumber(navigator.style.height) &&
        getMeNumber(navigator.style.left) < getMeNumber(ball.style.left) &&
        getMeNumber(ball.style.left) <
          getMeNumber(navigator.style.left) + getMeNumber(navigator.style.width)
      ) {
        dY = 1;
        if (
          getMeNumber(ball.style.left) <
          getMeNumber(navigator.style.left) +
            getMeNumber(navigator.style.width) / 2
        )
          dX = -1;
        else {
          dX = 1;
      ...
      if (
        getMeNumber(ball.style.left) >
        document.documentElement.scrollWidth - 2 * ball.offsetHeight
      ) {
        dX = -1;
      }
